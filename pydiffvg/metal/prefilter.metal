// Signed distance output and prefiltered colour for the diffvg Metal kernels
// (stage 3). Header order: common -> geometry -> color -> backward ->
// distance_grad -> prefilter.
//
// Ports of diffvg.cpp sample_distance and sample_color_prefiltered (forward
// and backward). Helpers here never touch output buffers (DESIGN.md
// "Mechanism"): gradients go through GradWrites, fragments through thread
// arrays; the kernel bodies (pydiffvg/render_metal_stage3.py) write outputs.
//
...
constant int PF_MAX_FRAGMENTS = 64;
constant int PF_SCENE_BVH_STACK = 64;

// opts[] slots (int32[8] input owned by render_metal_stage3.py)
constant int PF_OPT_WANT_TRANSLATION = 0;
constant int PF_OPT_EXACT_SCAN = 1;
...
inline float pf_d_filter_weight_dr(int type, float r, float dx, float dy) {
    float adx = fabs(dx);
    float ady = fabs(dy);
    if (adx > r || ady > r || !(r > 0)) {
        return 0.f;
    ...
    if (type == FILTER_BOX) {
        ...
        float fx = 0.5f * (1 - cos(2 * DIFFVG_PI * ndx));
        float fy = 0.5f * (1 - cos(2 * DIFFVG_PI * ndy));
        float dfx = 0.5f * sin(2 * DIFFVG_PI * ndx) * (2 * DIFFVG_PI) * (-dx / (2 * r * r));
        float dfy = 0.5f * sin(2 * DIFFVG_PI * ndy) * (2 * DIFFVG_PI) * (-dy / (2 * r * r));
...
inline bool sdf_query(SceneView s, float2 canvas_pt, thread SdfResult &r) {
    r.group_id = -1;
    r.shape_id = -1;
    r.distance = 0;
...
inline float pf_group_max_stroke_radius(SceneView s, int group_id) {
...
inline int pf_collect_fragments(SceneView s, float2 pt, bool exact_scan,
                                thread PrefilterFragment *fragments) {
    ...
    while (stack_size > 0) {
        ...
        if (child1 < 0) {
            ...
            if (group_has_stroke(s, group_id)) {
                ...
                if (!exact_scan) {
                    max_radius = min(pf_group_max_stroke_radius(s, group_id) + 1.f, DIFFVG_BIG);
                }
                if (compute_distance(s, group_id, pt, max_radius, sid, cp, info, d) && sid >= 0) {
                    float radius = path_thickness_at(s, sid, info);
                    float w = smoothstep(fabs(d) + radius) - smoothstep(fabs(d) - radius);
...
inline float4 pf_blend(thread PrefilterFragment *fragments, int num_fragments, bool has_bg, float4 bg,
                       thread float3 *accum_color, thread float *accum_alpha) {
    if (num_fragments <= 0) {
...
inline void pf_d_fragment(SceneView s, PrefilterFragment f, float2 pt, float3 d_color_i, float d_alpha_i,
                          thread GradWrites &g, thread float2 &d_translation) {
    int group_id = f.group_id;
    float d = f.distance;
    if (f.is_stroke) {
        float radius = path_thickness_at(s, f.shape_id, f.info);
        float a = fabs(d) + radius;
        float b = fabs(d) - radius;
        ...
        if (w != 0) {
            ...
            if (fabs(d_d) > 1e-10f && f.within_distance && f.shape_id >= 0) {
