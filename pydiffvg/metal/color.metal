// Colour sampling for the diffvg Metal kernels.
//
// Ports of sample_color(ColorType, ...) and the scene-level sample_color
// (fragment collection over the scene BVH + back-to-front blend) from
// diffvg.cpp. Built on common.metal and geometry.metal (is_inside,
// within_distance, EdgeQuery).
//
// Rules (DESIGN.md "Mechanism"): float32 only, math_mode "fast" (no inf/NaN
// logic, guarded divisions), helpers never touch output buffers.

constant int MAX_HIT_SHAPES = 256;
constant int MAX_SCENE_BVH_STACK = 64;

// ---------------------------------------------------------------------------
// Per-colour sampling: diffvg.cpp sample_color(ColorType, void*, pt)
// type/off/nstops come from the group record (fill_* or stroke_*).
inline float4 color_stop(SceneView s, int off, int nstops, int i) {
    int o = off + 4 + nstops + 4 * i;
    return float4(s.f[o], s.f[o + 1], s.f[o + 2], s.f[o + 3]);
}

inline float4 sample_gradient_stops(SceneView s, int off, int nstops, float t) {
    if (nstops <= 0) {
        return float4(0);
    }
    if (t < s.f[off + 4]) {
...
inline float4 sample_color_param(SceneView s, int type, int off, int nstops, float2 pt) {
    if (type == COLOR_CONSTANT) {
        ...
        float t = dot(pt - beg, end - beg) / max(dot(end - beg, end - beg), 1e-3f);
        ...
        float rx = fabs(radius.x) > 1e-30f ? radius.x : 1e-30f;
        float ry = fabs(radius.y) > 1e-30f ? radius.y : 1e-30f;
...
// fragments must point to a thread-local array of MAX_HIT_SHAPES entries.
// With use_edge_query, within_distance / is_inside update eq.hit as on the CPU.
inline int collect_fragments(SceneView s, float2 pt, bool use_edge_query, thread EdgeQuery &eq,
                             thread Fragment *fragments) {
...
inline float4 sample_color_scene_eq(SceneView s, bool has_bg, float4 bg, float2 screen_pt,
                                    bool use_edge_query, thread EdgeQuery &eq) {
    if (use_edge_query) {
        eq.hit = false;
    }
...
inline float4 blend_forward(thread Fragment *fragments, int num_fragments, bool has_bg, float4 bg,
                            float4 d_color, thread BlendState &st) {
...
inline float4 blend_backward_step(thread Fragment *fragments, int i, thread BlendState &st) {
    float3 prev_color = i > 0 ? st.accum_color[i - 1] : st.first_color;
    float prev_alpha = i > 0 ? st.accum_alpha[i - 1] : st.first_alpha;
...
inline float4 sample_color_scene(SceneView s, bool has_bg, float4 bg, float2 screen_pt) {
    EdgeQuery eq;
    eq.shape_group_id = -1;
    eq.shape_id = -1;
    eq.hit = false;
