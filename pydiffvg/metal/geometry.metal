// Geometry queries for the diffvg Metal kernels (stage 1: forward only).
//
// Ports of winding_number.h, is_inside (diffvg.cpp), compute_distance.h
// (closest_point + compute_distance) and within_distance.h. Built on
// common.metal (SceneView, BVH/shape accessors, AABB, Mat3, DIFFVG_BIG).
//
// Rules this file follows (see DESIGN.md "Mechanism"):
//  * float32 only (no double), no recursion, fixed-size stacks as in C++;
//  * kernels are compiled with math_mode "fast": no inf/NaN logic, every
//    division / sqrt / acos / pow argument is guarded explicitly;
...
//     curves split into y-monotone pieces at the roots of y'(t), one root per
//     straddling piece. The CPU runs it in double; here quadratics/cubics are
//     formed in Bernstein form in a frame centred on the query point and
//     scaled to O(1). Results differ only for points within float precision
//     of the curve.
...
//  3b. Performance (measured on tiger boundary samples, scratchpad geomperf/):
//     root refinements stop as soon as the Newton step is below 1e-7 (before,
//     a converged step rounding onto a bracket end was rejected and the
//     bracket was bisected to float exhaustion: ~12 wasted iterations per
//     root); wn_cubic_crossings returns [below0] - [below3] without splitting
//     when the whole hull is right of the point and decides each piece's
//     side from its x-hull (blossoms) before root solving; within_distance
//     rejects cubics with a split-at-1/2 capsule bound before the quintic.
//     is_inside results unchanged; within_distance and closest points differ
//     only within float rounding (|d - w| < 5e-4 at coordinates ~3000).
//  4. math_mode "fast" may reassociate float expressions everywhere, so even
//     the CPU-float code paths (lines, quadratics, circles, rects) can differ
//     in the last bits; this only matters for points exactly on a boundary.
...
constant int GEOM_PATH_BVH_STACK = 128;
constant int GEOM_GROUP_BVH_STACK = 64;

// ---------------------------------------------------------------------------
// solve.h (float instantiation, same branch structure)
inline bool solve_quadratic(float a, float b, float c, thread float &t0, thread float &t1) {
...
inline int solve_cubic(float a, float b, float c, float d,
                       thread float &t0, thread float &t1, thread float &t2) {
    if (fabs(a) < 1e-6f) {
    ...
    if (R * R < Q3) {
        ...
        float sQ = sqrt(max(Q, 0.f));
        float theta = acos(clamp(R / max(sqrt(Q3), 1e-30f), -1.f, 1.f));
        t0 = -2.f * sQ * cos(theta / 3.f) - b / 3.f;
        t1 = -2.f * sQ * cos((theta + 2.f * DIFFVG_PI) / 3.f) - b / 3.f;
        t2 = -2.f * sQ * cos((theta - 2.f * DIFFVG_PI) / 3.f) - b / 3.f;
        return 3;
    } else {
        float disc = sqrt(max(R * R - Q3, 0.f));
        float A = R > 0 ? -pow(max(R + disc, 0.f), 1.f / 3.f) :
                           pow(max(-R + disc, 0.f), 1.f / 3.f);
        float B = fabs(A) > 1e-6f ? Q / A : 0.f;
        t0 = (A + B) - b / 3.f;
        return 1;
    }
}
...
constant int BPOLY_MAX_DEPTH = 22;
constant int BPOLY_STACK = 32;
...
inline float bpoly_eval_d(BPoly p, float t, thread float &d) {
    float b[6];
    for (int i = 0; i < 6; i++) {
        b[i] = p.c[i];
    }
...
inline float bpoly_eval(BPoly p, float t) {
    float d = 0;
    return bpoly_eval_d(p, t, d);
}

inline void bpoly_split(BPoly p, thread BPoly &left, thread BPoly &right) {
    float b[6];
    for (int i = 0; i < 6; i++) {
        b[i] = p.c[i];
    }
...
inline int bpoly_sign_changes(BPoly p, thread float &first, thread float &last) {
    int changes = 0;
    first = 0;
    last = 0;
    for (int i = 0; i <= p.n; i++) {
...
inline float bpoly_refine01(BPoly p, float first) {
    bool neg_lo = first < 0;
    float lo = 0, hi = 1;
    float f0 = p.c[0], f1 = p.c[p.n];
    float u = 0.5f;
    if (f0 != 0 && f1 != 0 && (f0 < 0) != (f1 < 0)) {
        u = clamp(f0 / (f0 - f1), 0.f, 1.f);  // regula falsi start
    }
    for (int it = 0; it < 40; it++) {
        ...
        if (fabs(un - u) < 1e-7f || !(mid > lo && mid < hi)) {
            u = un;
            break;
        }
        u = un;
...
inline Roots5 bpoly_roots01(BPoly p) {
...
inline float bez3_eval_d(float4 c, float t, thread float &d) {
    float s = 1 - t;
...
inline int wn_quadratic_roots01(float a0, float a1, float a2, thread float &r0, thread float &r1) {
    int n = 0;
    float t0 = 0, t1 = 0;
    if (a0 == 0) {
        if (fabs(a2) < fabs(a1)) {
...
inline float wn_bez3_piece_root(float4 c, float lo, float hi, float ylo, float yhi, bool below_lo) {
    float u = 0.5f * (lo + hi);
    float den = ylo - yhi;
    ...
    for (int it = 0; it < 48; it++) {
        ...
        if (fabs(fu) < fabs(du) * (hi - lo)) {
            ...
            float nt = u - step;
            if (fabs(step) < 1e-7f) {
                return clamp(nt, lo, hi);  // converged (see bpoly_refine01)
            }
            if (nt > lo && nt < hi) {
                un = nt;
            }
...
inline float bez3_blossom(float4 c, float u1, float u2, float u3) {
...
inline int wn_cubic_crossings(float4 y, float4 x, bool below0, bool below3) {
    float xmax = max(max(x[0], x[1]), max(x[2], x[3]));
    if (!(xmax > 0)) {
        return 0;  // the curve lies in the half plane x <= 0
    }
    if (min(min(x[0], x[1]), min(x[2], x[3])) > 0) {
    ...
    for (int k = 0; k + 1 < n; k++) {
        ...
        bool right;
        if (min(min(h0, h1), min(h2, h3)) > 0) {
            right = true;
        } else if (!(max(max(h0, h1), max(h2, h3)) > 0)) {
            right = false;
        } else {
...
inline float2 eval_quadratic(float2 p0, float2 p1, float2 p2, float t) {
    float tt = 1 - t;
    return (tt * tt) * p0 + (2 * tt * t) * p1 + (t * t) * p2;
}

inline float2 eval_cubic(float2 p0, float2 p1, float2 p2, float2 p3, float t) {
    float tt = 1 - t;
    return (tt * tt * tt) * p0 + (3 * tt * tt * t) * p1 + (3 * tt * t * t) * p2 + (t * t * t) * p3;
}

inline float max_abs4(float2 a, float2 b, float2 c, float2 d) {
    float m = max(fabs(a.x), fabs(a.y));
    m = max(m, max(fabs(b.x), fabs(b.y)));
    m = max(m, max(fabs(c.x), fabs(c.y)));
    m = max(m, max(fabs(d.x), fabs(d.y)));
    return m;
}

inline float dist_point_segment(float2 p, float2 a, float2 b) {
    float2 ab = b - a;
    float ll = dot(ab, ab);
    float t = ll > 0 ? clamp(dot(p - a, ab) / ll, 0.f, 1.f) : 0.f;
...
inline float cubic_distance_lower_bound(float2 p0, float2 p1, float2 p2, float2 p3, float2 pt) {
    float h = max(dist_point_segment(p1, p0, p3), dist_point_segment(p2, p0, p3));
...
inline float cubic_split_lower_bound(float2 p0, float2 p1, float2 p2, float2 p3, float2 pt) {
    ...
    return min(cubic_distance_lower_bound(p0, m01, m012, mid, pt),
               cubic_distance_lower_bound(mid, m123, m23, p3, pt));
}

// F32: roots in [0, 1] of (q(t) - pt) . q'(t) for the cubic q (deviation 2).
inline Roots5 cubic_closest_roots(float2 p0, float2 p1, float2 p2, float2 p3, float2 pt) {
    float2 P0 = p0 - pt;
    float2 P1 = p1 - pt;
    float2 P2 = p2 - pt;
    float2 P3 = p3 - pt;
...
inline float det2(float2 a, float2 b) {
    return a.x * b.y - b.x * a.y;
}

inline float2 quadratic_closest_pt_approx(float2 p0, float2 p1, float2 p2, float2 pt, thread float &t_out) {
    ...
    float denom = 2 * a + b + d;
    float t = clamp(denom != 0 ? (ap + bp) / denom : 0.f, 0.f, 1.f);
    float tt = 1 - t;
    t_out = t;
...
// ---------------------------------------------------------------------------
// winding_number.h
inline int winding_number_circle(SceneView s, int sid, float2 pt) {
    float2 c = circle_center(s, sid);
    float r = circle_radius(s, sid);
    return distance_squared(c, pt) < r * r ? 1 : 0;
}

inline int winding_number_ellipse(SceneView s, int sid, float2 pt) {
...
inline int winding_number_path(SceneView s, int sid, float2 pt) {
...
inline int compute_winding_number(SceneView s, int shape_id, float2 local_pt) {
...
inline bool is_inside(SceneView s, int group_id, float2 canvas_pt, bool use_edge_query, thread EdgeQuery &eq) {
        ...
        if (child1 < 0) {
            ...
            if (use_edge_query) {
                if (eq.shape_group_id == group_id && eq.shape_id == shape_id) {
                    if ((even_odd && abs(w) % 2 == 1) || (!even_odd && w != 0)) {
                        eq.hit = true;
                    }
                }
            }
...
inline bool closest_point_circle(SceneView s, int sid, float2 pt, thread float2 &result) {
    float2 c = circle_center(s, sid);
    float r = circle_radius(s, sid);
    float2 dir = pt - c;
    ...
    result = c + r * (dir / sqrt(l2));
    return true;
}

// F32 (deviation 3): Eberly's root, bisected on u = s + 1.
inline float ellipse_get_root_u(float r0, float r0m1, float z0, float z1, float g) {
    float n0 = r0 * z0;
    float u0 = z1;                                         // s0 + 1
    float u1 = g < 0 ? 1.f : sqrt(n0 * n0 + z1 * z1);      // s1 + 1
    float u = u0;
    for (int i = 0; i < 200; i++) {
        u = 0.5f * (u0 + u1);
        if (u == u0 || u == u1) {
...
inline bool closest_point_ellipse(SceneView s, int sid, float2 pt, thread float2 &result) {
    float2 radius = ellipse_radius(s, sid);
    float2 center = ellipse_center(s, sid);
    float e[2] = {fabs(radius.x), fabs(radius.y)};
    ...
    float y0 = fabs(u[i0]), y1 = fabs(u[i1]);
    float x0 = 0, x1 = 0;
    if (y1 > 0) {
        if (y0 > 0) {
            float z0 = y0 / e0;
        ...
        if (numer0 < denom0) {
            ...
            x0 = e0 * xde0;
            x1 = e1 * sqrt(max(1 - xde0 * xde0, 0.f));
        } else {
            x0 = e0;
            x1 = 0;
        }
...
inline bool closest_point_rect(SceneView s, int sid, float2 pt, thread float2 &result) {
    float2 p_min = rect_p_min(s, sid);
    float2 p_max = rect_p_max(s, sid);
...
inline bool closest_point_path(SceneView s, int sid, float2 pt, float max_radius,
                               thread ClosestPointPathInfo &path_info, thread float2 &result) {
...
inline bool closest_point(SceneView s, int shape_id, float2 pt, float max_radius,
                          thread ClosestPointPathInfo &path_info, thread float2 &result) {
    int type = shape_type(s, shape_id);
    if (type == SHAPE_CIRCLE) {
...
inline bool compute_distance(SceneView s, int group_id, float2 canvas_pt, float max_radius,
                             thread int &min_shape_id, thread float2 &closest_pt,
                             thread ClosestPointPathInfo &path_info, thread float &distance_out) {
    // pt is in canvas space, transform it to shape's local space
    ...
    if (max_radius < DIFFVG_BIG) {
        ...
        float lambda_max = half_tr + sqrt(max(half_tr * half_tr - det, 0.f));
        local_max_radius = min(max_radius * sqrt(max(lambda_max, 0.f)), DIFFVG_BIG);
    }
...
// ---------------------------------------------------------------------------
// within_distance.h
inline bool within_distance_circle(SceneView s, int sid, float2 pt, float r) {
    float dist_to_center = distance(circle_center(s, sid), pt);
    return fabs(dist_to_center - circle_radius(s, sid)) < r;
}

inline bool within_distance_ellipse(SceneView s, int sid, float2 pt, float r) {
    // Cheap rejection using the annulus bounds min(|rx|,|ry|) .. max(|rx|,|ry|).
    float2 radius = ellipse_radius(s, sid);
    float d_center = distance(ellipse_center(s, sid), pt);
    float r_min = min(fabs(radius.x), fabs(radius.y));
    float r_max = max(fabs(radius.x), fabs(radius.y));
...
inline bool within_distance_segment(float2 p0, float2 p1, float2 pt, float r0, float r1) {
    float ll = dot(p1 - p0, p1 - p0);
    if (ll == 0) {
...
inline bool within_distance_path(SceneView s, int sid, float2 pt, float r) {
    int num_points = path_num_points(s, sid);
    ...
    while (stack_size > 0) {
        ...
        if (child1 < 0) {
            ...
            if (ncp == 0) {
                ...
                float rmax = max(max(fabs(r0), fabs(r1)), max(fabs(r2), fabs(r3)));
                Roots5 roots = roots5_empty();
...
inline bool within_distance_rect(SceneView s, int sid, float2 pt, float r) {
...
inline bool within_distance_shape(SceneView s, int shape_id, float2 pt, float r) {
    int type = shape_type(s, shape_id);
    if (type == SHAPE_CIRCLE) {
...
inline bool within_distance(SceneView s, int group_id, float2 canvas_pt, bool use_edge_query, thread EdgeQuery &eq) {
