// Distance derivatives for the diffvg Metal kernels (stage 3 leaves).
//
// Ports of diffvg.cpp smoothstep / d_smoothstep / prefilter_stroke_radius (+ its
// backward) and compute_distance.h d_closest_point / d_compute_distance.
// Header order: common -> geometry -> color -> backward -> distance_grad.
//
// Rules (DESIGN.md "Mechanism"): float32 only, math_mode "fast" (no inf/NaN
// logic, guarded divisions), no output buffers: every gradient is pushed into a
// GradWrites at the pool index of its value (mirror pool).
//
...
// finite differences of geometry.metal compute_distance):
...
// ---------------------------------------------------------------------------
// diffvg.cpp smoothstep / d_smoothstep (1-argument overloads; the Metal
// built-in smoothstep(edge0, edge1, x) is unaffected)
inline float smoothstep(float d) {
    float t = clamp((d + 1.f) / 2.f, 0.f, 1.f);
    return t * t * (3 - 2 * t);
}

inline float d_smoothstep(float d, float d_ret) {
    if (d < -1.f || d > 1.f) {
        return 0.f;
    }
    float t = (d + 1.f) / 2.f;
...
inline float path_thickness_at(SceneView s, int shape_id, ClosestPointPathInfo info) {
...
inline float path_thickness_dt_at(SceneView s, int shape_id, ClosestPointPathInfo info) {
    if (shape_type(s, shape_id) != SHAPE_PATH || !path_has_thickness(s, shape_id) ||
        info.base_point_id < 0) {
        return 0.f;
    }
...
inline void d_path_thickness_at(SceneView s, int shape_id, ClosestPointPathInfo info, float d_w,
                                thread GradWrites &g) {
    if (shape_type(s, shape_id) != SHAPE_PATH || !path_has_thickness(s, shape_id) ||
        info.base_point_id < 0) {
...
inline void d_closest_point_circle(SceneView s, int sid, float2 pt, float2 d_q,
                                   thread GradWrites &g, thread float2 &d_pt) {
...
inline void d_closest_point_ellipse(SceneView s, int sid, float2 pt, float2 d_q,
                                    thread GradWrites &g, thread float2 &d_pt) {
    ...
    float2 rel = cp - c;
    float theta = atan2(rel.y / b, rel.x / a);
    float ct = cos(theta);
    float st = sin(theta);
    float2 u = pt - c;
    float2 d_c = d_q;
...
inline void dg_d_project_segment(float2 p0, float2 p1, float2 pt, float t, float2 d_q, float d_t_extra,
                                 thread float2 &d_p0, thread float2 &d_p1, thread float2 &d_pt) {
    float2 e = p1 - p0;
    float ll = dot(e, e);
    if (t < 0 || !(ll > 0)) {
        d_p0 += d_q;
...
inline void d_closest_point_rect(SceneView s, int sid, float2 pt, float2 d_q,
                                 thread GradWrites &g, thread float2 &d_pt) {
...
inline void d_closest_point_path(SceneView s, int sid, float2 pt, float2 d_q, ClosestPointPathInfo info,
                                 float d_t_root, thread GradWrites &g, thread float2 &d_pt) {
    ...
    if (n == 1) {
        ...
        if (fabs(G_t) > 1e-6f * scale && scale > 0) {
...
inline void d_closest_point(SceneView s, int shape_id, float2 pt, float2 d_closest_pt,
                            ClosestPointPathInfo info, float d_t_root, thread GradWrites &g,
                            thread float2 &d_pt) {
...
inline void d_closest_point(SceneView s, int shape_id, float2 pt, float2 d_closest_pt,
                            ClosestPointPathInfo info, thread GradWrites &g, thread float2 &d_pt) {
...
inline Mat3 dg_mat3_mul(Mat3 a, Mat3 b) {
    Mat3 r;
...
inline Mat3 dg_mat3_transpose(Mat3 a) {
    Mat3 r;
...
inline void d_compute_distance(SceneView s, int group_id, int shape_id, float2 canvas_pt, float2 closest_pt,
                               ClosestPointPathInfo info, float d_dist, float d_t_root, thread GradWrites &g,
    ...
    float2 d_v = (d_dist / sqrt(l2)) * diff;
    float2 d_pt = d_v;
...
inline void d_compute_distance(SceneView s, int group_id, int shape_id, float2 canvas_pt, float2 closest_pt,
                               ClosestPointPathInfo info, float d_dist, thread GradWrites &g,
                               thread float2 &d_translation) {
