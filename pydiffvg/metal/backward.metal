// Backward-pass leaf functions for the diffvg Metal kernels.
//
// Header order: common.metal, geometry.metal, color.metal, backward.metal.
// Only common.metal is required by this file.
//
// Mirrors (float32, GradWrites instead of atomic_add):
...
// inf/NaN, which fast math may not handle):
...
inline BoundaryData boundary_data_zero() {
    BoundaryData d;
...
inline void d_sample_color_param(SceneView s, int type, int off, int nstops, float2 pt,
                                 float4 d_color, thread GradWrites &g,
                                 thread float2 &d_translation) {
...
// d_render_image (W*H*4) and weight_image (W*H) must be kernel INPUT buffers.
inline float4 gather_d_color(SceneView s, const device float *d_render_image,
                             const device float *weight_image, float2 pt) {
    int width = s.ip[IP_W];
...
inline int cdf_sample(SceneView s, int off, int n, float u, thread float &updated_u) {
    const device float *cdf = s.f;
    int lb = 0;
    int len = n - 1 - lb;
    while (len > 0) {
        int half_len = len / 2;
...
inline float path_shape_length(SceneView s, int sid) {
...
// ---------------------------------------------------------------------------
// sample_boundary.h
inline float offset_curve_speed(float2 d1, float2 d2, float r, float dr, float dir) {
    float len = length(d1);
    if (!(len > 0)) {
        return 0;
    }
...
inline float2 offset_curve_normal(float2 d1, float2 d2, float r, float dr, float dir, float2 n) {
    float len = length(d1);
    if (!(len > 0)) {
        return n;
    }
...
inline float2 sample_boundary_circle(SceneView s, int sid, float t, thread float2 &normal,
                                     thread float &pdf, float dir, float stroke_radius) {
    ...
    float boundary_radius = fabs(r + dir * stroke_radius);
...
inline float2 sample_boundary_ellipse(SceneView s, int sid, float t, thread float2 &normal,
                                      thread float &pdf, thread BoundaryData &data,
    ...
    float tan_len = sqrt(square(dxdt) + square(dydt));
    if (!(tan_len > 0)) {
        pdf = 0;
        return float2(0);
    }
...
inline int path_vertex_point_id(SceneView s, int sid, int k) {
    return k < path_num_base_points(s, sid) ? path_point_id_map(s, sid, k) : path_num_points(s, sid) - 1;
}

inline float2 sample_boundary_path(SceneView s, int sid, float t, thread float2 &normal,
                                   thread float &pdf, thread BoundaryData &data,
                                   float dir, float stroke_radius) {
    int nbp = path_num_base_points(s, sid);
    int np = path_num_points(s, sid);
    ...
    if (dir != 0.f) {
        ...
        if (t < cap_prob) {
            t = t / cap_prob;
            int k = min(int(t * num_circles), num_circles - 1);
            t = t * num_circles - k;
...
inline float2 sample_boundary_rect(SceneView s, int sid, float t, thread float2 &normal,
                                   thread float &pdf, float dir, float stroke_radius) {
...
inline float2 sample_boundary(SceneView s, int group_id, int shape_id, float t,
                              thread float2 &normal, thread float &pdf,
                              thread BoundaryData &data) {
...
inline bool generate_boundary_sample(SceneView s, int idx, thread BoundarySample &b) {
    b.pt = float2(0);
    b.local_pt = float2(0);
    b.normal = float2(0);
...
inline void accumulate_boundary_gradient(SceneView s, int shape_id, float canvas_contrib, float t,
                                         float2 local_normal, BoundaryData data, int group_id,
                                         float2 local_boundary_pt, float2 canvas_normal,
                                         float local_velocity_scale, thread GradWrites &g) {
    (void)t;
    ...
    if (type == SHAPE_CIRCLE) {
        ...
        gw_push(g, f_off + 1, cos(a) * normal.x * contrib);
        gw_push(g, f_off + 2, sin(a) * normal.y * contrib);
    } else if (type == SHAPE_PATH) {
